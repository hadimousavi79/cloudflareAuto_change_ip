...
def initialize_env():
    if not os.path.exists('.env'):
        ...
        with open('.env', 'w') as env_file:
            ...
            api_token = input("Enter your Cloudflare Global API token: ")
...
ADDRESSES = []
ZONE_IDS = []
MAX_ATTEMPTS = 3  # تعداد دفعات مجاز برای تلاش جهت بررسی سرور
INTERVAL = int(os.getenv('INTERVAL', 120))  # زمان بررسی پیش‌فرض یا مشخص‌شده توسط کاربر

# Load zone and server information
...
def check_subdomain_status(zone_id, subdomain, ip, last_status, change_summary, status_summary):
    ...
    if ping_time is None:
        ...
        if subdomain_status['ping_failures'] >= MAX_ATTEMPTS:
            new_ip = None
            sorted_addresses = sorted(ADDRESSES, key=lambda x: x[2])  # مرتب‌سازی بر اساس priority
...
def check_for_revert_to_original_ip(zone_id, subdomain, last_status, change_summary):
    ...
    if subdomain_status['new_ip'] is not None:
        ...
        if successful_pings and successful_tcps:
            ...
            change_summary.append(f"✅ {subdomain} (IP: {original_ip}) - Successfully reverted to original IP after recovery.")
            write_status_file(last_status)
            ...
            if record['name'] == subdomain:
                ...
                if update_dns_record(zone_id, record_id, subdomain, new_ip):
                    if new_ip == subdomain_status['original_ip']:
                        ...
                        change_summary.append(f"❌ {subdomain} (IP: {subdomain_status['original_ip']}) - Updated to new IP: {new_ip}")
                    break
        write_status_file(last_status)
    else:
